...
import { AlertCircle, AlertTriangle, Box, Check, CircleHelp, FileCode2, FileJson2, GitFork, Info, ShieldAlert } from 'lucide-react';
...
export function ManifestResourceInspector({ selected, selectedEdges, selectedInsights, nodeByKey, onSelect, onOpenInEditor, className = '' }: Props) {
  ...
  return <Card className={`min-h-0 border-border bg-card ${className}`}><CardHeader className="pb-2"><div className="flex items-start justify-between gap-2"><div className="min-w-0"><CardTitle className="truncate text-sm">{selected.name}</CardTitle><CardDescription className="mt-0.5 truncate">{selected.kind} · {selected.namespace ?? 'cluster-scoped'}</CardDescription></div><div className="flex shrink-0 items-center gap-1.5"><Badge variant="outline" className="text-[9px]">{selected.apiVersion}</Badge>{onOpenInEditor && <Button variant="outline" size="sm" className="h-6 gap-1 px-2 text-[10px]" onClick={() => onOpenInEditor(selected.source.filePath, selected.source.line, selected.source.column)} title={`Open ${selected.source.filePath}${selected.source.line ? `:${selected.source.line}` : ''} in VS Code`}><FileCode2 size={11}/><span>Open</span></Button>}</div></div></CardHeader><CardContent className="h-[calc(100%-72px)] p-0"><ScrollArea className="h-full px-3 pb-3"><Tabs defaultValue="overview">
    ...
    <TabsContent value="overview" className="mt-3 space-y-3"><div className="grid grid-cols-3 gap-2">{[{ label: 'Incoming', value: incoming, icon: GitFork }, { label: 'Outgoing', value: outgoing, icon: GitFork }, { label: 'Containers', value: selected.containers.length, icon: Box }].map(item => <div key={item.label} className="rounded-md border border-border bg-muted/30 p-2"><item.icon size={12} className="text-muted-foreground"/><p className="mt-1 text-sm font-semibold tabular-nums">{item.value}</p><p className="text-[9px] text-muted-foreground">{item.label}</p></div>)}</div><dl className="space-y-2 rounded-md border border-border bg-muted/20 p-3 text-[10px]"><div><dt className="text-muted-foreground">Canonical key</dt><dd className="mt-0.5 break-all font-mono">{selected.key}</dd></div><div><dt className="text-muted-foreground">Source</dt><dd className="mt-0.5 flex items-center justify-between gap-2 break-all font-mono text-[10px]"><span>{selected.source.filePath}{selected.source.line ? `:${selected.source.line}` : ''}</span>{onOpenInEditor && <Button variant="ghost" size="sm" className="h-5 px-1.5 text-[10px] text-primary" onClick={() => onOpenInEditor(selected.source.filePath, selected.source.line, selected.source.column)} title="Open file in VS Code"><FileCode2 size={10} className="mr-1"/>Open</Button>}</dd></div>{selected.source.chartName && <div><dt className="text-muted-foreground">Helm chart</dt><dd className="mt-0.5">{selected.source.chartName}</dd></div>}</dl><p className="text-[10px] text-muted-foreground">This inspector describes uploaded desired state. It does not claim live cluster health.</p></TabsContent>
    <TabsContent value="relations" className="mt-3">{selectedEdges.map((edge, index) => { const isOutgoing = edge.from === selected.key; const otherKey = isOutgoing ? edge.to : edge.from; const other = nodeByKey.get(otherKey); return <button key={`${edge.from}-${edge.to}-${index}`} type="button" onClick={() => onSelect(otherKey)} className="mb-2 w-full rounded-md border border-border bg-muted/30 p-2 text-left hover:bg-muted"><div className="flex items-center justify-between gap-2"><Badge variant="outline" className="text-[9px]">{relationVerb(edge, isOutgoing)}</Badge><div className="flex items-center gap-1.5">{edge.meta?.containerName && <span className="truncate text-[10px] text-primary">{String(edge.meta.containerName)}</span>}{onOpenInEditor && <span role="button" tabIndex={0} className="inline-flex cursor-pointer items-center gap-1 rounded bg-muted/70 px-1.5 py-0.5 text-[9px] text-primary transition-colors hover:bg-primary/15 hover:text-primary" onClick={(e) => { e.stopPropagation(); const targetFile = isOutgoing ? selected.source.filePath : (other?.source.filePath ?? selected.source.filePath); const targetLine = typeof edge.meta?.line === 'number' ? edge.meta.line : undefined; onOpenInEditor(targetFile, targetLine); }} title={`Open connection line in VS Code (${isOutgoing ? selected.source.filePath : (other?.source.filePath ?? selected.source.filePath)}${edge.meta?.line ? `:${edge.meta.line}` : ''})`}><FileCode2 size={10}/><span>{typeof edge.meta?.line === 'number' ? `Line ${edge.meta.line}` : 'Open'}</span></span>}</div></div><p className="mt-2 truncate text-xs font-medium">{other ? `${other.kind} / ${other.name}` : otherKey}</p><p className="mt-0.5 truncate text-[9px] text-muted-foreground">{other?.namespace ?? 'cluster-scoped'} · {edge.type === 'literal:name' ? 'inferred evidence' : edge.type.includes('Selector') ? 'selector evidence' : 'declared field'}</p>{edge.meta?.fieldPath && <p className="mt-1 truncate font-mono text-[9px] text-muted-foreground">{String(edge.meta.fieldPath)}</p>}</button>;})}{selectedEdges.length === 0 && <p className="text-xs text-muted-foreground">No resolved relations for this object in the upload.</p>}</TabsContent>
