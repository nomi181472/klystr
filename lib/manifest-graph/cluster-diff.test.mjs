...
test('detects nested property drift in embedded JSON inside ConfigMap data (state: STARTING vs RUNNING)', () => {
  ...
  // Verify diffSummary highlights the exact drifted nested property and indicates redeployment requirement
  const summaryMatches = comparison.diffSummary.some(
    s => s.includes('config.json.state') && s.includes('RUNNING') && s.includes('STARTING') && s.includes('needs to be redeployed')
  );
  assert.ok(summaryMatches, 'Expected diffSummary to mention config.json.state and redeployment requirement');
...
test('detects added property in file missing in cluster (POSTGRES_DB_TEST env var)', () => {
  const manifest = createMockNode('StatefulSet', 'sts-bookstore-db', 'klystr-bookstore', {
    apiVersion: 'apps/v1',
    kind: 'StatefulSet',
    metadata: { name: 'sts-bookstore-db', namespace: 'klystr-bookstore' },
    spec: {
      template: {
        spec: {
          containers: [
            {
              ...
              env: [
                ...
                { name: 'POSTGRES_DB_TEST', value: 'bookstore' }, // Added in file
  ...
  const cluster = createMockResource('StatefulSet', 'sts-bookstore-db', 'klystr-bookstore', {
    apiVersion: 'apps/v1',
    kind: 'StatefulSet',
    metadata: { name: 'sts-bookstore-db', namespace: 'klystr-bookstore' },
    spec: {
  ...
  const comparison = compareObject(manifest, cluster);
  assert.equal(comparison.status, 'out-of-sync');
  assert.equal(comparison.statusLabel, 'Needs Redeployment');

  const addedMatch = comparison.diffSummary.some(
    s => s.includes('POSTGRES_DB_TEST') && s.includes('needs to be redeployed')
  );
  assert.ok(addedMatch, 'Expected diffSummary to report POSTGRES_DB_TEST added in file needs to be redeployed');
});
